...
function respondHtmlFile(readFile, req, res){
  ...
  .then(function(buff){
    ...
    var rawString = buff.toString()
    var insertIndex = rawString.search(/<script/)
    var tag = '<script src="reload/reload.js"></script>'
    
    if(insertIndex>0){
      var fileContents = rawString.substring(0,insertIndex) + tag + rawString.substring(insertIndex,rawString.length)
    }else{
      var fileContents = rawString + tag
    }
